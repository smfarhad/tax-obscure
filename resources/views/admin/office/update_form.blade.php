                    ...
                    <div class="box-body">
                        ...
                        <div class="box-body">                            
                            <div class="form-group">
                                <label for="head_office" class="col-sm-2 control-label">Head Office</label>
                                <div class="col-sm-5">
                                    <select name="parent" class="form-control">
                                         <option value="">Select...</option>
                                        @if (count($data['office']) > 0)
                                            @foreach($data['office'] as $row)
                                                <option value="{{$row->id}}" @if($row->id == $show->parent_id) {{ "selected" }} @endif >{{$row->name}}</option>
                                            @endforeach
                                        @endif
                                    </select> 
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-5">
                                    <input name="order_by" value="{{ $show->order_by }}" class="form-control" id="name" placeholder="Order By" type="number" required>
                                </div>
                            </div>                                                                   
                        </div>
                        <!-- /.box-body -->
